...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        all_sites_launches = spacex_df['Launch Site'].value_counts()

        fig = px.pie(names=all_sites_launches.index,
                     values=all_sites_launches.values,
                     title='Total Launches for All Sites')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        site_success = filtered_df[filtered_df['class'] == 1].shape[0]
        site_failed = filtered_df[filtered_df['class'] == 0].shape[0]
